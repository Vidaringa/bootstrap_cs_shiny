    ...
    div(
        ...
        fluidRow(
            column(
                ...
                class = "well",
                # Toggle Button
                actionButton(inputId = "toggle_form", label = "Toggle Form"),

                # Controls
                div(
                  id = "controls",
                  br(),
                  shiny::textInput(inputId = "first_name", label = "First Name", placeholder = "Enter Your First Namse"),
...
server <- function(input, output, session) {
    ...
    # 16 ShinyJS ----
    # similar to conditionalpanel from shiny
    shinyjs::onclick(id = "toggle_form", {
        shinyjs::toggle(id = "controls", anim = TRUE)
    })

    observe({
        shinyjs::toggleState(id = "submit_form",
                             condition = {
